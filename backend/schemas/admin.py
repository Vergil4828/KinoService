...
class AdminChangePlanRequest(BaseModel):
    ...
    price: float = Field(..., ge=0, description="Цена подписки", examples=[999])
    renewalPeriod: int = Field(
        default=30, ge=1, description="Период подписки в днях", examples=[30]
    )
            ...
            if "endDate" in v and v["endDate"]:
                try:
                    if isinstance(v["endDate"], str):
                        v["endDate"] = datetime.fromisoformat(
                            v["endDate"].replace("Z", "+00:00")
                        )
                except ValueError:
