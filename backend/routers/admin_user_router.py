...
from backend.core.dependencies import get_admin_user, log_admin_action
...
@router.get("/users", dependencies=[Depends(get_admin_user)], summary="Получить список пользователей")
async def get_users_route(
    page: int = Query(1, ge=1, description="Номер страницы"),
    limit: int = Query(25, ge=0, description="Количество пользователей на странице (0 для всех)"),
...
async def change_user_route(
    ...
    **Возвращает:**
    ...
    """
    return await AdminUserService.admin_change_user(userId, request_data, request, admin_user)
