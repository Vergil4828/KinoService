...
from fastapi import HTTPException, status, Request, Depends # HTTPException и status для обработки ошибок, Request для доступа к заголовкам, Depends для внедрения зависимостей
from motor.motor_asyncio import AsyncIOMotorClient # Для работы с клиентом Motor и транзакциями
from pymongo.errors import DuplicateKeyError # Для перехвата ошибки дублирования ключа при создании плана
...
from backend.schemas.admin import AdminChangePlanRequest # Модель запроса для изменения/создания плана
from backend.core.dependencies import log_admin_action # Ваша утилита для логирования действий администратора
from backend.core.database import get_motor_client # Ваша функция для получения клиента Motor (для транзакций)
from backend.core.dependencies import get_admin_user # Ваша зависимость для получения администратора
