...
from beanie import (
    PydanticObjectId,
)  # Для работы с ID документов MongoDB (PydanticObjectId) # Для зависимостей FastAPI
...
from backend.schemas.token import (
    RefreshTokenRequest,
)  # Схема для запроса refresh-токена
...
)  # Вложенная модель для refresh-токена
        ...
        **Возвращает:**
        ...
        try:
            ...
            is_match = bcrypt.checkpw(
                request_data.password.encode("utf-8"), admin.password.encode("utf-8")
            )
            if not is_match:
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
            ...
            tokens = await generate_tokens(admin)
            admin_agent = request.headers.get("user-agent", "Unknown")
            refresh_token_data = RefreshTokenEmbedded(
                token=tokens["refreshToken"], userAgent=admin_agent
            )

            await User.find_one(User.id == admin.id).update(
                {"$push": {"refreshTokens": refresh_token_data}}
            )
            ...
            raise HTTPException(
                ...
                detail="Ошибка сервера при логине администратора",
            )
        ...
        try:
            ...
            try:
                ...
                if not user_id or role != "admin":
                    logger.warning(
                        f"Invalid refresh token payload: userId={user_id}, role={role}"
                    )
            ...
            if not admin or admin.role != "admin":
                ...
                logger.warning(
                    f"User with ID {user_id} not found or not admin during refresh token."
                )
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
            ...
            try:
                ...
                logger.error(
                    ...
                    "Another process might have modified the document.",
                    exc_info=True,
                )
                raise HTTPException(
            ...
            logger.error(
                f"Critical error during admin refresh token process: {e}", exc_info=True
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        **Возвращает:**
        ...
        try:
            ...
            if update_result.modified_count == 0:
                user_with_token = await User.find_one(
                    {"refreshTokens.token": request_data.refreshToken}
                )
                if not user_with_token:
                    raise HTTPException(
                        status_code=status.HTTP_404_NOT_FOUND,
                        detail="Refresh токен не найден или уже недействителен",
                    )
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="Ошибка сервера при выходе администратора",
            )
