        ...
        try:
                ...
                {"$addFields": {
                    "currentSubscription.plan": {
                        "$cond": {
                            ...
                            "then": {"$arrayElemAt": ["$currentSubscription.plan", 0]},
                            "else": None
                        }
                    },
                    "wallet": {
            ...
            for user in users_list:
                ...
                if user.get("currentSubscription"):
                    ...
                    plan_id_str = str(sub_data["planId"]) if sub_data.get("planId") else None

                    user_data["currentSubscription"] = {
                        "planId": plan_id_str,
                        "startDate": sub_data.get("startDate").isoformat() if sub_data.get("startDate") else None,
                        "endDate": sub_data.get("endDate").isoformat() if sub_data.get("endDate") else None,
    ...
    async def admin_change_user(
        ...
        **Возвращает:**
        ...
        try:
            ...
            logger.error(f"MongoDB client not initialized for admin_change_user: {e}", exc_info=False)
                ...
                try:
                    ...
                    if request_data.email is not None and request_data.email != user.email:
                        try:
                            ...
                            raise HTTPException(status_code=500, detail="Internal server error during email validation")

                    if request_data.wallet is not None and 'balance' in request_data.wallet:
                        try:
                    ...
                    if request_data.currentSubscription is not None:
                        ...
                        if 'planId' in sub_data:
                            if sub_data['planId'] is None or sub_data['planId'] == "":
                                ...
                                except (ValidationError, beanie_exceptions.DocumentNotFound, ValueError) as e:
                                    logger.error(f"Invalid planId format or plan not found: {sub_data['planId']} - {e}", exc_info=True)
                                    raise HTTPException(status_code=400, detail="Invalid planId format or plan not found")

                        simple_fields = ['isActive', 'autoRenew', 'adminNote', 'startDate', 'endDate']
                        for field in simple_fields:
                            if field in sub_data:
                                new_subscription[field] = sub_data[field]

                        if new_subscription:
                            current_sub = user.currentSubscription.model_dump(by_alias=True) if user.currentSubscription else {}
                            merged_sub = {**current_sub, **new_subscription}
                            
                            if 'planId' in merged_sub and merged_sub['planId'] is not None:
                                merged_sub['planId'] = PydanticObjectId(merged_sub['planId'])
    ...
    async def admin_delete_user(
        ...
        **Возвращает:**
        ...
        try:
            ...
            logger.error(f"MongoDB client not initialized for admin_delete_user: {e}", exc_info=False)
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Database not available")
