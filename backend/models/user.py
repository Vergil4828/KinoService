...
class User(Document):
    ...
    currentSubscription: Optional[CurrentSubscriptionEmbedded] = Field(
        default_factory=CurrentSubscriptionEmbedded
    )
    wallet: WalletEmbedded = Field(default_factory=WalletEmbedded)
    role: str = Field(
        default="user", description="User role", examples=["user", "admin"]
    )
    ...
    class Settings:
        ...
        indexes = [
            ...
            IndexModel(
                ...
                name="refreshTokens.createdAt_1",
                background=True,
                expireAfterSeconds=604800,
            ),
            IndexModel(
                [("refreshTokens.token", 1)],
                name="refreshTokens_token_1",
                background=True,
            ),
        ]
