...
class TestLoginUserPositive:
    ...
    async def test_login_user(
        ...
        status_code,
        ids,
        registered_user_in_db,
        clean_all_users,
    ):
        await registered_user_in_db(
            {
                "username": "user_now",
                "email": email,
                "password": password,
        ...
        response = await api_client.login_user(credential)
...
class TestLoginUserValidValidation:
    ...
    async def test_login_user_valid_password(
        ...
        response = await api_client.login_user(credential)
        assert response.status_code == status_code
        await clean_all_users("user@example.com")
...
class TestLoginUserInvalidValidation:
    ...
    async def test_login_user_invalid_email(self, api_client, email, status_code, ids):
    ...
    async def test_login_user_invalid_password(
        self, api_client, password, status_code, ids
    ):
        credential = LoginUserData.base_credential.copy()
...
class TestLoginUserNegative:
    @pytest.mark.asyncio
    async def test_login_user_with_not_exists_email_in_database(self, api_client):
        response = await api_client.login_user(LoginUserData.base_credential.copy())
        assert response.status_code == 401
        assert response.json()["detail"] == "Invalid email or password"

    @pytest.mark.asyncio
    async def test_login_user_with_wrong_password_for_email_in_db(
        self, api_client, registered_user_in_db
    ):
        credential = LoginUserData.base_credential.copy()
        await registered_user_in_db(
            {
    ...
    async def test_login_user_missing_required_field(
        self, api_client, field_to_remove, status_code
    ):
        credential = LoginUserData.base_credential.copy()
        del credential[field_to_remove]
        response = await api_client.login_user(credential)
        print(response.json())  # проверить, что 422 именно из-за отсутствия рек.поля
        assert response.status_code == status_code

    @pytest.mark.asyncio
    async def test_login_user_with_duplicate_field(
        self, api_client, clean_all_users, registered_user_in_db
    ):
        await registered_user_in_db(
            {
                "username": "dupli_user",
